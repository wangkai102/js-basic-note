...
- async 函数返回一个 Promise 对象，当函数执行的时候，一旦遇到 await 就 会先返回，等到触发的异步操作完成，再执行函数体内后面的语句。可以理解为， 是让出了线程，跳出了 async 函数体。
...
11. 减少回流与重绘
    ...
    回流：改变DOM元素大小及位置时，会导致浏览器重新生成渲染树，这个过程叫回流

    重绘：当DOM元素的属性改变、或者样式改变而不会影响布局的过程称为重绘
